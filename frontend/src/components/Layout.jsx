...
export default function Layout() {
  ...
  const userRole = 'admin'; // Mantendo como admin para você usar o seletor de visão
  ...
  return (
    <div className={`flex h-screen bg-[#020617] text-gray-100 font-sans antialiased selection:bg-amber-500/30 ${viewMode === 'mobile' ? 'justify-center items-center bg-black' : ''}`}>
      ...
      {/* 🛠️ SELETOR DE VISÃO (DIFERENCIAL SAE) */}
      {userRole === 'admin' && (
        <div className="fixed top-6 right-6 z-[100] bg-[#1e293b]/80 backdrop-blur-xl p-1.5 rounded-2xl border border-white/5 flex shadow-[0_0_30px_rgba(0,0,0,0.5)]">
          ...
          <button onClick={() => setViewMode('mobile')} className={`px-5 py-2 text-[10px] font-black rounded-xl transition-all duration-300 ${viewMode === 'mobile' ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.4)]' : 'text-gray-500 hover:text-gray-300'}`}>MOBILE</button>
        </div>
      )}
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.path} to={item.path} 
                  ...
                  ${isActive ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'text-gray-400 hover:text-gray-100 hover:bg-white/5'}`}>
                  {isActive && <div className="absolute left-0 top-1/4 h-1/2 w-1 bg-amber-500 rounded-r-full"></div>}
                  <span className={`text-xl ${isActive ? 'filter drop-shadow-[0_0_5px_rgba(245,158,11,0.5)]' : ''}`}>{item.icon}</span>
      ...
      <div className="md:hidden fixed top-0 left-0 w-full h-20 bg-[#020617]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-8 z-[60]">
        ...
        <button onClick={() => setIsMobileMenuOpen(true)} className="w-10 h-10 flex items-center justify-center bg-amber-500/10 rounded-xl text-amber-500 border border-amber-500/20 text-2xl">
          ☰
        </button>
      </div>
      ...
      <div className={`md:hidden fixed inset-0 z-[70] transition-all duration-500 ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        <div className={`absolute inset-0 bg-black/60 backdrop-blur-md transition-opacity ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'}`} onClick={() => setIsMobileMenuOpen(false)}></div>
        <nav className={`absolute top-0 right-0 w-[80%] h-full bg-[#0f172a] shadow-[-20px_0_50px_rgba(0,0,0,0.5)] flex flex-col p-10 transition-transform duration-500 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex flex-col gap-2 overflow-y-auto">
            {navItems.map((item) => (
              <Link key={item.path} to={item.path} className={`flex items-center gap-5 p-5 rounded-2xl text-xs font-black uppercase tracking-[0.2em] ${location.pathname === item.path ? 'bg-amber-500 text-black' : 'text-gray-400'}`}>
      ...
      <main className={`flex-1 overflow-y-auto bg-[#020617] relative w-full scroll-smooth
        ...
        <div className={`${maxWidthClass} mx-auto w-full flex flex-col gap-12 relative z-10`}>
          <Outlet /> {/* Aqui o React injeta o conteúdo das páginas */}
        </div>
